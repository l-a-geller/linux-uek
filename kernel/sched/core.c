...
static inline bool __sched_core_less(struct task_struct *a, struct task_struct *b)
{
	...
	if (prio_less(b, a, task_rq(a)->rke->core->rke->core_forceidle))
		return true;
...
static struct task_struct *sched_core_find(struct rq *rq, unsigned long cookie)
{
	...
	node = rb_find_first((void *)cookie, &rq->rke->core_tree, rb_sched_core_cmp);
	/*
...
static void __sched_core_flip(bool enabled)
{
	...
	for_each_cpu(cpu, &sched_core_mask) {
		...
		for_each_cpu(t, smt_mask)
			cpu_rq(t)->rke->core_enabled = enabled;
...
static void sched_core_assert_empty(void)
{
	...
	for_each_possible_cpu(cpu)
		WARN_ON_ONCE(!RB_EMPTY_ROOT(&cpu_rq(cpu)->rke->core_tree));
}
...
#else
		raw_spin_lock_nested(&rq->lock, subclass);
#endif
...
pick_next_task(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if (rq->rke->core->rke->core_forceidle) {
		need_sync = true;
		fi_before = true;
		rq->rke->core->rke->core_forceidle = false;
	}
...
again:
		for_each_cpu_wrap(i, smt_mask, cpu) {
			...
			if (rq_i->idle == p && rq_i->nr_running) {
				rq->rke->core->rke->core_forceidle = true;
				if (!fi_before)
					rq->rke->core->rke->core_forceidle_seq++;
			}
	...
	for_each_cpu(i, smt_mask) {
		 ...
		 *  1            1       0
		 */
		if (!(fi_before && rq->rke->core->rke->core_forceidle))
			task_vruntime_update(rq_i, rq_i->rke->core_pick,
					     rq->rke->core->rke->core_forceidle);
...
static void sched_core_cpu_starting(unsigned int cpu)
{
	...
	for_each_cpu(t, smt_mask) {
		...
		WARN_ON_ONCE(rq->rke->core != core_rq);
	}
...
static void sched_core_cpu_deactivate(unsigned int cpu)
{
	...
	core_rq->rke->core_task_seq      = rq->rke->core_task_seq;
	core_rq->rke->core_pick_seq      = rq->rke->core_pick_seq;
	core_rq->rke->core_cookie        = rq->rke->core_cookie;
	core_rq->rke->core_forceidle     = rq->rke->core_forceidle;
	core_rq->rke->core_forceidle_seq = rq->rke->core_forceidle_seq;
