...
static inline void task_tick_core(struct rq *rq, struct task_struct *curr)
{
	 ...
	 */
	if (rq->rke->core->rke->core_forceidle && rq->cfs.nr_running == 1 &&
...
void task_vruntime_update(struct rq *rq, struct task_struct *p, bool in_fi)
{
	...
	se_fi_update(se, rq->rke->core->rke->core_forceidle_seq, in_fi);
}
...
bool cfs_prio_less(struct task_struct *a, struct task_struct *b, bool in_fi)
{
	...
	SCHED_WARN_ON(task_rq(b)->rke->core != rq->rke->core);
	...
	se_fi_update(sea, rq->rke->core->rke->core_forceidle_seq, in_fi);
	se_fi_update(seb, rq->rke->core->rke->core_forceidle_seq, in_fi);
